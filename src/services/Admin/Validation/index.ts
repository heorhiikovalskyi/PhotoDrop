...
class AdminValidationService {
  ...
  public json = async (token: unknown) => {
    ...
    if (typeof decoded === 'string' || decoded.role !== 'admin') throw new AuthorizationError('not valid token');
    return { id: decoded.id, role: decoded.role } as { id: number; role: 'admin' };
  };
}
