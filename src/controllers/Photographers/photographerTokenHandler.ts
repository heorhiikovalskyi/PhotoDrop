...
export const photographerTokenHandler = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (typeof decoded === 'string' || decoded.role !== 'photographer') throw new AuthorizationError('token is not valid');

  res.locals.user = { id: decoded.id, role: decoded.role } as { id: number; role: 'photographer' };
